...
<div class = "login_box">
    ...
    <input type="text" name="username" maxlength="50" placeholder="ユーザー名(本名)" class = "form_box" id ="username" autofocus required />
   
    <select name = "company" class = "form_box" id = "company">
        <option value = "0" label = "--会社--">--会社--</option>
    	<?php foreach($companies as $company){?>
    		<option value = "<?php echo $company->id; ?>" label = "<?php echo $company->name; ?>"><?php echo $company->name; ?></option>
        <?php } ?>
    </select>
    ...
    <select name = "domain" class = "form_box domain_box">
    	<?php foreach($companies as $company){?>
    		<option value = "@<?php echo $company->domain; ?>" label = "@<?php echo $company->domain; ?>">@<?php echo $company->domain; ?></option>
        <?php } ?>
    </select>
